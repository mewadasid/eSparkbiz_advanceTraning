...
function Post() {
    ...
    const fetchPost = async () => {
        try {
            const response = await fetch(`https://jsonplaceholder.typicode.com/posts?userId=${userId.state}`);
    ...
    function loadPost() {
        setVisible((prevvisible) => { return prevvisible += 3 })
    }
    console.log(visible);
    ...
    return (
        <>
            <div className="container">
                <Link to={'/user'}><button className="back_btn">Back to User</button></Link>
